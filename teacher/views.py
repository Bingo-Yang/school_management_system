...
def showtea_view(request):
    if request.method=='GET':
        ...
        return render(request,'teacher.html',{'cose':cose})
    else:
        #教师编号
        ...
        return HttpResponse('<script>alert("关联教师任职表成功！点击确定返回页面");location.href="/teach/coursetea/";</script>')
...
def guide_view(request):
    if request.method=='GET':
        ...
        return render(request,'guide.html',{'cls':cls})
    else:
        #请输入教师编号
        ...
        return HttpResponse('<script>alert("关联班主任任职表成功！点击确定返回页面！");location.href="/teach/coursetea/";</script>')
...
def coursetea_view(request):
    if request.method=='GET':
        ...
        try:
            ...
            return HttpResponse('<script>alert("查询失败!请重试！");location.href="/teach/coursetea/";</script>')
        ...
        try:
            ...
            if query == '年级':
                ...
                for cl in clazz:
                    ...
                    tal.append(clz2)
                return render(request, 'show2_guide.html', {'tal': tal})
        except:
            return HttpResponse('<script>alert("查询失败!请重试！");location.href="/teach/coursetea/";</script>')
